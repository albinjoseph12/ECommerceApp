...
//import com.ecommerce.shopping.R; // Corrected import
import com.eshopthis.finds.ItemView;
import com.eshopthis.finds.db.Model; // Assuming this is the correct location
...
public class Adapter extends RecyclerView.Adapter<Adapter.ImageViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ImageViewHolder holder, int position) {
        ...
        holder.tvPrice.setText(temp.getPrice() + " $");

        // Load images using Glide
        if (temp.getImageUrl() != null && !temp.getImageUrl().isEmpty()) {
            Glide.with(context).load(temp.getImageUrl()).into(holder.imageView);
        } else if (temp.getImageResId() != 0) {
            holder.imageView.setImageResource(temp.getImageResId());
        }
        ...
        holder.constraintLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    ...
                    // Determine what to pass based on what's available
                    if (temp.getImageUrl() != null && !temp.getImageUrl().isEmpty()) {
                        intent.putExtra("imageData", temp.getImageUrl());
                    } else if (temp.getImageResId() != 0) {
                        intent.putExtra("imageData", temp.getImageResId());
                    }
